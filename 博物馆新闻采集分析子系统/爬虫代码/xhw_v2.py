...
def crawl_news(url_list, keyword):
    ...
    all_news = {}
    # all_news["name"] = keyword  # 不再需要
    all_news["data"] = []
    # all_news["valid_num"] = 0  # 有效新闻的数量  # 不再需要
    titles = []  # 有些新闻是重复的，要去重
    ...
    for url in url_list:
        ...
        try:
            ...
            # all_news["valid_num"] += 1  # 不再需要

        except Exception as e:
            print(e)
    # valid_num = all_news["valid_num"]  # 不再需要
    # 所谓“处理”就是过滤掉非主站新闻，因为非主站（地方频道、其他子频道）新闻的页面布局各不相同，一个个的话写时间成本高
    # print(f"处理过后还剩{valid_num}条新闻  :(")  # 不再需要
    # print(all_news)
    if len(all_news["data"]) > 0:  # 只有存在新闻才创建json文件，避免导入数据库的时候出错
