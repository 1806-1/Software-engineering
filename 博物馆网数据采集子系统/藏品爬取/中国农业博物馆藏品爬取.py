...
# 博物馆藏品列表页网址
url = "https://www.ciae.com.cn/list/zh/visit.html"
...
for kk in links:  # 遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_='maintxt'):  # 详情页活动介绍正文
        ...
        for a in img_link:  # 遍历img字段
            ImgList.append("https://www.ciae.com.cn/list/" + a['src'])  # 网页给的img链接没有"http://www.sxhm.com/"自己加上
            print("https://www.ciae.com.cn/list/" + a['src'])
            break  # 只取第一张图片
        i = 0  # 计数
...
dataframe.to_csv(r"中国农业博物馆藏品.csv", sep=',')
