...
# 博物馆藏品列表页网址
url = "http://www.xbpjng.cn/wenbotiandi/chenlie.aspx?c=2ab3fba2-98d3-4d7c-9eb0-8cfe6d982a71&z=d58e507b-b470-4b40-abe1-8756985e8456"
...
for kk in links:  # 遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_='maintxt'):  # 详情页活动介绍正文
        ...
        for a in img_link:  # 遍历img字段
            ImgList.append("http://www.xbpjng.cn/wenbotiandi/" + a['src'])  # 网页给的img链接没有"http://www.sxhm.com/"自己加上
            print("http://www.xbpjng.cn/wenbotiandi/" + a['src'])
            break  # 只取第一张图片
        i = 0  # 计数
        for tag in Detailsoup.select('p', calss_='MsoNormal'):  # <p class="MsoNormal">字段是文字介绍
            ...
            print("Introduce1", Introduce)
            # IntroList.append(Introduce)
            # print(Introduce)
            if (len(Introduce) >15):  # 大于5个字的保存并且结束（即只保存第一句）
                i = i + 1
                IntroList.append(Introduce)
...
dataframe.to_csv(r"西柏坡纪念馆藏品.csv", sep=',')
