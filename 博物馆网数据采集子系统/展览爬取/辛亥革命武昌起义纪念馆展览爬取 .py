...
for kk in links:#遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_ = 'yknr_mav'):#详情页活动介绍正文
        ...
        for a in img_link:#遍历img字段
            ImgList.append("http://wlt.hubei.gov.cn/1911museum/" + a['href'])#网页给的img链接没有"http://www.sxhm.com/"自己加上
            print("http://wlt.hubei.gov.cn/1911museum/" + a['href'])
            break#只取第一张图片
        i = 0#计数
...
for i in range(0, len(Name_Act_List)):
    ...
    Museum_list.append("辛亥革命武昌起义纪念馆")
...
dataframe.to_csv(r"辛亥革命武昌起义纪念馆活动.csv",sep=',')
