...
# 博物馆藏品列表页网址
url = "http://www.bjp.org.cn/cgzn/cgzn/list.shtml"
...
for kk in links:  # 遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_='maintxt'):  # 详情页活动介绍正文
        ...
        for a in img_link:  # 遍历img字段
            ImgList.append("http://www.bjp.org.cn/cgzn/" + a['src'])
            print("http://www.bjp.org.cn/cgzn/cgzn/" + a['src'])
            break  # 只取第一张图片
        i = 0  # 计数
...
dataframe.to_csv(r"北京天文馆.csv", sep=',')
