...
for kk in links:#遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_ = 'yknr_mav'):#详情页活动介绍正文
        ...
        for a in img_link:#遍历img字段
            ImgList.append("http://www.potalapalace.cn/" + a['href'])#网页给的img链接没有"http://www.sxhm.com/"自己加上
            print("http://www.potalapalace.cn/" + a['href'])
            break#只取第一张图片
        i = 0#计数
...
dataframe.to_csv(r"布达拉宫活动.csv",sep=',')
