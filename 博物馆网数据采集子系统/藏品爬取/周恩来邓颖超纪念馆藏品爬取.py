...
# 博物馆藏品列表页网址
url = "http://www.sxhm.com/index.php?ac=article&at=list&tid=218"
...
for kk in links:  # 遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_='maintxt'):  # 详情页活动介绍正文
        ...
        for a in img_link:  # 遍历img字段
            ImgList.append("http://news.enorth.com.cn/" + a['src'])  # 网页给的img链接没有"http://www.sxhm.com/"自己加上
            print("http://news.enorth.com.cn/" + a['src'])
            break  # 只取第一张图片
        i = 0  # 计数
...
dataframe.to_csv(r"周恩来邓颖超纪念馆藏品.csv", sep=',')
