...
# 博物馆藏品列表页网址
url = "http://ordosbwg.org.cn/zlcl_121914/"
...
for kk in links:  # 遍历详情页链接
    ...
    for tag in Detailsoup.findAll('div', class_='maintxt'):  # 详情页活动介绍正文
        ...
        for a in img_link:  # 遍历img字段
            ImgList.append("http://ordosbwg.org.cn/" + a['src'])  # 网页给的img链接没有"http://www.sxhm.com/"自己加上
            print("http://ordosbwg.org.cn" + a['src'])
            break  # 只取第一张图片
        i = 0  # 计数
...
dataframe.to_csv(r"鄂尔多斯博物馆藏品.csv", sep=',')
